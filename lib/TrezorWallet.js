                    ...
                    while (1) {
                        switch (_context2.prev = _context2.next) {
                            ...
                            case 5:
                                ...
                                _connect2.default.TrezorConnect.getXPubKey(path, function (response) {
                                    if (response.success) {
                                        // success
                                        var hdWallet = _hdkey2.default.fromExtendedKey(response.xpubkey);
                                        var addr = hdWallet.getWallet().getAddressString();
                                        addresses.push(addr);
                                        addressToPathMap[addr] = path;

                                        for (var i = 0; i < 5; i++) {
                                            var address = hdWallet.deriveChild(i).getWallet().getAddressString();
                                            ...
                                            while (1) {
                                                switch (_context3.prev = _context3.next) {
                                                    case 0:
                                                        ...
                                                        _connect2.default.TrezorConnect.ethereumSignTx(addressToPathMap[txData.from.toLowerCase()], stripAndPad(txData.nonce), stripAndPad(txData.gasPrice), stripAndPad(txData.gas), stripAndPad(txData.to), stripAndPad(txData.value), stripAndPad(txData.data), chain_id, function (response) {
